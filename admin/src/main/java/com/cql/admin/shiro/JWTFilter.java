...
//        }
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        HttpServletRequest httpServletRequest = (HttpServletRequest) request;
        String auth = httpServletRequest.getHeader(JWTUtil.AUTH_HEADER);
        UsernamePasswordToken token = new UsernamePasswordToken(LoginType.JWT_LOGIN, auth, "", "");
        if (token == null) {
...
//            return onLoginSuccess(token, subject, request, response);
    ...
    protected AuthenticationToken createToken(ServletRequest servletRequest, ServletResponse servletResponse) {
        ...
        UsernamePasswordToken token = new UsernamePasswordToken(LoginType.JWT_LOGIN, authorization, "", "");
        return token;
    }
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        writer.write("{\"code\": -200, \"message\": \"未登录\"}");
    ...
    /**
     * Shiro 利用 JWT token 登录成功，会进入该方法 /刷新token有限期
     */
    @Override
    protected boolean onLoginSuccess(AuthenticationToken token, Subject subject, ServletRequest request,
        ...
        if (token instanceof UsernamePasswordToken) {
            if (((com.cql.commons.moudel.shiro.UsernamePasswordToken) token).getLoginType() == LoginType.JWT_LOGIN) {
                newToken = JWTUtil.refreshTokenExpired(((UsernamePasswordToken) token).getToken(), JWTUtil.TOKEN_SECRET);
            }
        }
        if (newToken != null) {
            httpResponse.setHeader(JWTUtil.AUTH_HEADER, newToken);
        }
        System.out.println(newToken.equals(((UsernamePasswordToken) token).getToken()));
        return true;
    }
